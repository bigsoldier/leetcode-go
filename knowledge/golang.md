...
只要满足下面两种情况之一，可包对象不丢失

* 弱三色不等式：所有被黑色对象引用的白色对象都处于灰色保护状态（黑色对象可以引用白色对象，但白色对象需要被灰色对象引用）
* 强三色不等式：不存在黑色到白色指针的对象

- 插入屏障：被黑色对象引用的对象强制变为灰色对象（满足强三色）
- 删除屏障：被删除的对象，如果本身是灰色或白色，则被标记为灰色（满足弱三色）

插入写屏障和删除写屏障的短板：
- Dijkstra插入写屏障：标记开始时无需STW，结束时仍需要STW来重新扫描栈，标记栈上引用的白色对象存活
- Yuasa删除写屏障：回收精度低，GC开始时STW扫描堆栈来记录初始快照，这个过程会保护开始时刻的所有存活对象。

**v1.8的混合写屏障（hybird write barrier）**（满足弱三色）：只需要在开始时并发扫描各个goroutine的栈，使其变黑并一直保持，这个过程不需要STW，
而标记结束后，因为栈在扫描后始终是黑色的，所以无需重复扫描，减少了STW时间。
...
2、有两个存储G的队列，一个是局部调度器P的本地队列，一个是全局G队列。新创建的G会先保存在P的本地队列中，如果本地队列满了会把本地队列的一半保存到全局队列中。
...
m0是go启动后编号为0的主线程，是全局变量，不需要在堆上分配，负责初始化和操作第一个g，也就是g0。
