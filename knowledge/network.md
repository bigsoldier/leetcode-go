...
## 15、http和https的区别
- http是超文本协议，信息是明文传输，https是具有安全性的ssl加密传输协议
- http和https使用完全不同的连接方式，使用的端口不一样，http是80，https是443
- http连接无状态，https=ssl+http

**流程**

1、客户端向服务器发起https请求，连接服务器的443端口

2、服务端有一个密钥对，即公钥和私钥（非对称加密），服务端保存着私钥，不对外泄露，将公钥发送给客户端

3、浏览器收到服务端发来的ssl证书，对证书的真伪进行校验，如果证书有问题，那么https传输无法继续。
如果公钥合格，那么客户端会生成一个随机值，用于对称加密client key，然后用公钥对client key进行非对称加密，这样客户端秘钥变成密文了。
用约定好的hash方式，对密文hash（主要作用是对密文做一个签名，验证密文没有被修改过）

4、客户端会发起https的第二个http请求，将加密后的客户端秘钥发送给服务器

5、服务器收到客户端发来的密文，用私钥对其进行非对称解密，解密后的明文就是客户端秘钥client key，然后用客户端秘钥对数据进行对称加密，这样数据就变成密文

6、服务器将加密后的密文发送给客户端

7、客户端收到服务器发送来的密文，用客户端秘钥对其对称解密，得到服务器发送的数据。

这样https的两次http请求结束，整个https传输完成

## 16、TCP拥塞控制

网络拥塞：在某段时间，对网络某一个资源的需求超过了该资源所能提供的可用部分，网络性能就会变坏。

![TCP拥塞](https://gitee.com/zongl/cloudImage/raw/master/images/2021/03/08/TCP拥塞.jpg)

TCP拥塞算法
- 慢开始
  * 当主机开始发送数据时，如果立即将较大的发送窗口的全部数据字节都注入网络，由于不清楚网络的状况，可能会引发网络拥塞
  * 比较好的方式是从小到大逐渐增大发送端的拥塞控制窗口数量，成指数上升
  * 还有一个ssthresh(slow start threshold)，是一个上限。当拥塞窗口cwnd>=ssthresh时，就会进入拥塞避免算法
- 拥塞避免：
  * cwnd成线性增长，即每往返一个时间RTT就会把发送方的拥塞窗口cwnd+1，可以避免拥塞窗口快速增长的问题。
  * 每收到一个ack时cwnd=cwnd+1/cwnd
- 快重传：用于丢包检测，以便能更快重传数据包，调整拥塞状态机的状态，从而达到持续升窗的目的
